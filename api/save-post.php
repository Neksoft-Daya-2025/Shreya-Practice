...
function ensureImageAltsInHtml($html, $fallbackAlt) {
    if ($html === '' || $html === null) {
        return $html;
    }
    $alt = htmlspecialchars($fallbackAlt !== '' ? $fallbackAlt : 'Ligen Power blog image', ENT_QUOTES, 'UTF-8');
    return preg_replace_callback('/<img\b[^>]*>/i', function ($matches) use ($alt) {
        $tag = $matches[0];
        if (preg_match('/\balt\s*=\s*(["\'])(.*?)\1/i', $tag, $altMatch) && trim($altMatch[2]) !== '') {
            return $tag;
        }
        if (preg_match('/\/>\s*$/', $tag)) {
            return preg_replace('/\/>\s*$/', ' alt="' . $alt . '" />', $tag);
        }
        return preg_replace('/>\s*$/', ' alt="' . $alt . '">', $tag);
    }, $html);
}
...
while (true) {
    ...
    foreach ($posts as $p) {
        ...
        if ($id !== null && isset($p['id']) && (string) $p['id'] === $id) {
            continue;
        }
        $slugTaken = true;
        break;
...
$post = [
    ...
    'content' => ensureImageAltsInHtml(isset($data['content']) ? $data['content'] : '', $title),
